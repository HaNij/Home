...
// Перечисления в Java являются ограниченными классами, т.е enum может иметь поля, методы, конструкторы, интерфейсы, но не может наследоваться.
enum Apple {
//    Для каждой константы вызывается свой конструктор, в этом примере вызывается конструктор, в котором указывается цена, для каждой константы.
...
//    Все перечисления наследуется от Enum и от Object соотвественно, значит мы можем переопределить метод класса Object - public String toString()
    @Override
    public String toString() {
//        Ссылка this используется для определения конкретного объекта класса (читай перечисления) Apple, который содержит какой-либо сорт яблока
        switch (this){
